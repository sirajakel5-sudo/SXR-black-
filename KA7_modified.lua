...
function redzlib:SetTheme(NewTheme)
	...
	SaveJson("Velmorith library V1.json", redzlib.Save)  -- NEXUS 
...
local WindowBackground = Create("ImageLabel", MainFrame, {
    ...
    BackgroundTransparency = 1,
    Image = "rbxassetid://136430835242724",
...
local StrokeGradient = Create("UIGradient", Stroke, {
    ...
    Color = ColorSequence.new({
        ...
        ColorSequenceKeypoint.new(0.25, Color3.fromRGB(70, 70, 70)),   -- رمادي
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 255, 255)), -- أبيض
        ColorSequenceKeypoint.new(0.75, Color3.fromRGB(70, 70, 70)),   -- رمادي
...
local function SpawnCyberpunkSystem()
	...
	function Window:CloseBtn()
		local Dialog = Window:Dialog({
			Title = "إغلاق",  
			Text = "هل حقا بدك تقفل سكربت ؟", -- رساله قفل الواجهه
			Options = {
				{"نعم", function()  -- باتن الموافقه ايزي يعني
					ScreenGui:Destroy()
				end},
				{"لا"} -- باتن عدم الموافقه علي القفل
			}
		})
	end
	function Window:MinimizeBtn()  -- انشاء ايقونه الواجهه تكون صغيره كذا و حلوه
		...
		function Tab:Addsizedropdown()
			return Tab:AddDropdown({
				...
				Callback = function(Value)
					...
					elseif Value == "متوسطة" then
						MainFrame.Size = UDim2.fromOffset(550, 380)
					elseif Value == "كبيرة" then
...
function Tab:AddSideDropdown()
    return Tab:AddDropdown({
        ...
        Callback = function(Value)
            ...
            elseif Value == "يمين" then
                MainScroll.Position = UDim2.new(1, 0, 1, 0)
                MainScroll.AnchorPoint = Vector2.new(1, 1)
                
                Containers.Position = UDim2.new(0, 0, 1, 0)
                Containers.AnchorPoint = Vector2.new(0, 1)
            end
        end
    })
end
	...
	function Tab:AddDiscordInvite(Configs)  -- ننشاء ديسكورد كذا كذا ما بنضيف لاكن نضيفه اذا احتجناه
...
pcall(function()
	...
	if MainFrame:IsA("ImageLabel") or MainFrame:IsA("ImageButton") then
		MainFrame.Image = "rbxassetid://87950815567502"
	end

	if XButton and XButton:IsA("ImageButton") then
		XButton.Image = ""
	end

	if YButton and YButton:IsA("ImageButton") then
		YButton.Image = ""
	end
	...
	if XButton then
		XButton.MouseButton1Click:Connect(function()
			...
			ConfirmFrame.Size = UDim2.new(0,180,0,80)
			ConfirmFrame.Position = UDim2.new(0.5,-90,0.5,-40)
			ConfirmFrame.BackgroundColor3 = Color3.fromRGB(15,15,15)
			ConfirmFrame.BorderColor3 = Color3.fromRGB(40,40,40)
			...
			CloseBtn.Parent = ConfirmFrame
			CloseBtn.Size = UDim2.new(0,70,0,28)
			CloseBtn.Position = UDim2.new(0,15,0,40)
			CloseBtn.Text = "Close"

			local CancelBtn = Instance.new("TextButton")
			CancelBtn.Parent = ConfirmFrame
			CancelBtn.Size = UDim2.new(0,70,0,28)
			CancelBtn.Position = UDim2.new(0,95,0,40)
